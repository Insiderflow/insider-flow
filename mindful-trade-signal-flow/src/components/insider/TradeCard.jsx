...
export default function TradeCard({ trade, type = 'politician', subtitleMode = 'default', subtitleSector, onClick }) {
  const { displayChamber, displayParty, displaySector, displayIssuerName, t } = useTranslation();
  ...
  const subtitle = isPolitician
    ? subtitleMode === 'sector'
      ? `${t('sector')}: ${displaySector(subtitleSector || trade.sector) || t('unknown')}`
      : `${displayParty(trade.party) || ''} · ${displayChamber(trade.chamber) || ''} · ${trade.state || ''}`
    : `${trade.title || ''} · ${displayIssuerName(trade.company_name, trade.ticker) || ''}`;
  ...
  return (
    <button
      onClick={onClick}
      className="w-full text-left bg-card rounded-xl border border-border/50 p-4 hover:border-border transition-all motion-press motion-elevate motion-fade-in"
    >
        ...
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="font-semibold text-ui-body truncate">{name}</span>
              {trade.notable && (
                <AlertTriangle className="h-3.5 w-3.5 text-warning-color flex-shrink-0" />
              )}
            </div>
            <p className="text-ui-meta truncate">{subtitle}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {trade.company_name && isPolitician && (
            <span className="text-xs text-muted-foreground truncate max-w-[120px]">
              {displayIssuerName(trade.company_name, trade.ticker)}
            </span>
          )}
        </div>
